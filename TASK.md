...
1. ✅ 6.1 Implement payload size validation
    1. ✅ 6.1.a Enforce max scenario input size (characters + estimated tokens)
    1. ✅ 6.1.b Enforce max dataset import size (row count + total payload)
    1. ✅ 6.1.c Return structured `StandardizedError` with code `INPUT_TOO_LARGE` on violation
1. ✅ 6.2 Implement prompt injection detection (server-side)
...
1. ✅ 6.4 Implement evaluator prompt builder with safety fencing
